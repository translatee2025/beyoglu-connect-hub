...
const Venues = () => {
  ...
  const mapPins = filtered.filter((v: any) => v.lat && v.lng).map((v: any) => ({
    lat: v.lat, lng: v.lng, title: v.name, badge: (v as any).venue_types?.name, extra: v.address,
  }));
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <Dialog open={postOpen} onOpenChange={setPostOpen}>
              <DialogTrigger asChild><Button className="gap-2"><Plus className="w-4 h-4" /> Add Venue</Button></DialogTrigger>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <Button variant={selectedType === "all" ? "default" : "outline"} size="sm" onClick={() => setSelectedType("all")}>All</Button>
...
const VenuePostForm = ({ venueTypes, onSuccess }: { venueTypes: any[]; onSuccess: () => void }) => {
  ...
  const toggleDay = (day: string) => {
    if (selectedDays.includes(day)) {
      ...
      setHours({ ...hours, [day]: { open: "09:00", close: "18:00" } });
    }
  };
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("venues").insert({
        ...
        venue_type_id: venueTypeId, created_by_user_id: user.id,
        photos: photos.length > 0 ? photos : null,
        hours: selectedDays.length > 0 ? hours : null,
      });
      if (error) throw error;
    },
    ...
    <div className="space-y-4">
      ...
      {step === 1 && (
        <div className="space-y-3">
          ...
          <div><Label>Description</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="What does this place offer?" rows={3} /></div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          <div>
            <Label className="flex items-center gap-1 mb-2"><Clock className="w-4 h-4" /> Opening Hours</Label>
            <div className="flex flex-wrap gap-2 mb-3">
              {WEEKDAYS.map((d) => (
                <Button key={d.key} type="button" variant={selectedDays.includes(d.key) ? "default" : "outline"} size="sm" onClick={() => toggleDay(d.key)}>{d.label}</Button>
              ))}
            </div>
      ...
      <div className="flex gap-2">
        ...
        {step < 2 ? (
          <Button className="flex-1 gap-1" onClick={() => setStep(2)} disabled={!form.name.trim()}>Next <ArrowRight className="w-4 h-4" /></Button>
        ) : (
          <Button className="flex-1" onClick={() => mutation.mutate()} disabled={!form.name || mutation.isPending}>{mutation.isPending ? "Adding..." : "Add Venue"}</Button>
        )}
      </div>
    </div>
  );
