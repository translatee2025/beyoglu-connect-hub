...
const defaultFilters: Filters = {
  species: "all", size: "all", energy_level: "all", gender: "all", personality: [], looking_for: "all",
};
                  ...
                  {Object.keys(personalityColors).map(tag => (
                    <button key={tag} type="button" onClick={() => togglePersonality(tag)}
                      className={`text-xs px-3 py-1.5 rounded-full border transition-colors capitalize ${
                        filters.personality.includes(tag) ? "bg-primary text-primary-foreground border-primary" : "bg-muted text-muted-foreground border-border hover:border-primary"
                      }`}>{tag}</button>
                  ))}
                </div>
              </div>
...
const FriendCard = ({ pet, onWoof, speciesEmojiMap }: { pet: any; onWoof: (pet: any) => void; speciesEmojiMap: Record<string, string> }) => (
  <Card className="hover:shadow-lg transition-all hover:-translate-y-0.5 border-border/50 overflow-hidden">
    ...
    <CardContent className="p-4">
      ...
      <p className="text-sm text-muted-foreground">{pet.breed || pet.species}{pet.age_years ? ` • ${pet.age_years}y` : ""}</p>
      {pet.neighborhood && <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1"><MapPin className="w-3 h-3" />{pet.neighborhood}</div>}
      {pet.bio && <p className="text-sm text-foreground/80 mt-2 line-clamp-2">{pet.bio}</p>}
      {pet.personality_tags?.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          {pet.personality_tags.slice(0, 3).map((tag: string) => (
            <span key={tag} className={`text-[11px] px-2 py-0.5 rounded-full border ${personalityColors[tag] || "bg-muted text-muted-foreground"}`}>{tag}</span>
          ))}
        </div>
      )}
      <Button onClick={() => onWoof(pet)} variant="default" size="sm" className="w-full gap-2 mt-3">
