...
const MobileDrawer = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  return (
    <>
      {open && (
        <div className="fixed inset-0 z-[60] bg-black/50 lg:hidden" onClick={onClose} />
      )}
      <div
        className={`fixed top-0 right-0 h-full z-[70] bg-white flex flex-col transition-transform duration-300 lg:hidden ${open ? "translate-x-0" : "translate-x-full"}`}
        style={{ width: 280 }}
      >
            ...
            <div key={section.label}>
              ...
              {section.items.map((item) => (
                <button
                  ...
                  style={{ padding: "9px 12px", borderRadius: 8, fontSize: 13, color: "#64748B" }}
                >
